...
func sendStoragePage(b *Bot, chatID int64, sess *Session, sendFiles bool) {
	...
	if sendFiles {
		...
		lastMsgID := sendPageFiles(b, chatID, pageItems)
		...
		if nextStart < len(sess.Items) {
			...
			go prefetchItems(b, sess.Items[nextStart:nextEnd])
		}
		if lastMsgID != 0 {
			...
			if _, err := b.api.Send(editKb); err != nil {
				...
				lastMsgID = editOrSendHTML(b, chatID, 0, sb.String(), &kb)
			}
			sess.LastMsgID = lastMsgID
		} else {
			// No media was sent (empty page or all failed) — send a plain text message.
			sess.LastMsgID = editOrSendHTML(b, chatID, 0, sb.String(), &kb)
		}
	} else {
		// No new media — edit the existing control message if possible.
		sess.LastMsgID = editOrSendHTML(b, chatID, sess.LastMsgID, sb.String(), &kb)
	}
}
